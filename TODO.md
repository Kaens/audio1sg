...
This is where you can contribute the most! (sorted by extension)
 ...
 - Digital Symphony `.DSYM` (size,may involve unLZing)
 ...
 - FastTracker 2 `.XM` (fix the ADPCM calcsize)
 - Pixel Painters `.FMF`, `.FTF`
 - FM-Kingtracker `.FMK` (info, size)
 - FM Tracker `.FMT` (info, size -- all three of them)
 ...
 - Fast Tracker `.FTC` (count some more info)
 - FamiTracker `.FTM` (info, size)
 - Flash Tracker `.FLS` (redo the algo)
 ...
 - Packen 714MIDIJr `.INT` (compare MUAP98)
 - Packen 714MIDISpecial `.INT` (compare MUAP98)
 ...
 - MVS `.MUS` (info, size)
 - MVX `.MVM` (size might be too hard to do, it changed per demo)
 ...
 - MSX ProTracker `.PRO`
 - PreTracker `.PRT` (size)
 - Pro Sound Creator `.PSC` (find orns, count notes?)
 ...
 - Westwood `.SND`
 - `.SNDH` (size might not be possible)
 ...
 - Symphonie `.SYMMOD` (fix, a bit more info, confirm about size)
